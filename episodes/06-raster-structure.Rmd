...
# Extract just the red band for plotting
red_band <- RGB_2m[[1]]
names(red_band) <- "red"

ggplot() +
  geom_spatraster(data = red_band, show.legend = FALSE) +
...
# Use plotRGB to display the RGB image
plotRGB(RGB_2m, r = 1, g = 2, b = 3)
title(main = "Orthographic Imagery", sub = "All bands")
...
# Extract blue band
blue_band_nas <- RGB_2m_nas[[3]]

ggplot() +
  geom_spatraster(data = blue_band_nas) +
...
# memory saving
rm(RGB_2m, rgb_stack, red_band, RGB_2m_nas, blue_band_nas)
```
...
dsm_high <- classify(dsm_harv,
                         rcl = matrix(c(0, 400, 1, 400, 420, 2),
                                      ncol = 3, byrow = TRUE),
                         include.lowest = TRUE)
...
ggplot() +
  geom_spatraster(data = dsm_harv) +
  scale_fill_viridis_c() +
  geom_spatraster(data = dsm_high, fill = "deeppink", alpha = 0.5) +
