...
to use the `crs()` function to extract the CRS in `proj4` format from that
...
We will use the `vect()` function to perform the conversion.
...
<img src="fig/05-vector-csv-to-shapefile-in-r-rendered-plot-data-points-1.png" alt="" style="display: block; margin: auto;" />
...
``` r
ggplot() +
  geom_spatvector(data = aoi_boundary_harv) +
  geom_spatvector(data = plot_locations_sp_harv) +
  ggtitle("AOI Boundary Plot")
```

<img src="fig/05-vector-csv-to-shapefile-in-r-rendered-plot-data-1.png" alt="" style="display: block; margin: auto;" />
...
CRS from the `us_outline` object and confirm that it is WGS84.
...
Then we will convert our new data to a SpatVector, using the `wgs_84_crs`
object as our CRS.
...
<img src="fig/05-vector-csv-to-shapefile-in-r-rendered-plot-locations-harv-orange-1.png" alt="" style="display: block; margin: auto;" />
...
We can write an R spatial object to an ESRI `shapefile` using the `writeVector` function
...
``` r
writeVector(plot_locations_sp_harv,
...
- Know the projection (if any) of your point data prior to converting to a
  spatial object.
- Convert a data frame to a `SpatVector` object using the `vect()` function with the `geom` parameter.
- Export a `SpatVector` object as a shapefile using the `writeVector()` function.
