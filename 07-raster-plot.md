...
quickly change the palette across all plots by modifying the `my_colors` object, 
...
the `geom_raster()` function. Let's overlay `dsm_harv` on top of the `dsm_hill_harv`.
